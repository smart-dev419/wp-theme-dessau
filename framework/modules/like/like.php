...
class DessauSelectLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_dessau_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_dessau_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'dessau' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'dessau' );
		}
...
if ( ! function_exists( 'dessau_select_create_like' ) ) {
	function dessau_select_create_like() {
		DessauSelectLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'dessau_select_create_like' );
}