...
if ( ! function_exists( 'dessau_select_vertical_menu_styles' ) ) {
	function dessau_select_vertical_menu_styles() {
		...
		$vertical_background_color = dessau_select_options()->getOptionValue( 'vertical_header_background_color' );
		$vertical_background_image = dessau_select_options()->getOptionValue( 'vertical_header_background_image' );
		$vertical_shadow_enabled   = dessau_select_options()->getOptionValue( 'vertical_header_shadow' );
		$vertical_border_enabled   = dessau_select_options()->getOptionValue( 'vertical_header_border' );
		...
		if ( $vertical_border_enabled === 'yes' ) {
			$header_border_color = dessau_select_options()->getOptionValue( 'vertical_header_border_color' );
		...
		echo dessau_select_dynamic_css( $vertical_header_selectors, $vertical_header_styles );
	}
	
	add_action( 'dessau_select_style_dynamic', 'dessau_select_vertical_menu_styles' );
}

if ( ! function_exists( 'dessau_select_vertical_main_menu_styles' ) ) {
	/**
	 * Generates styles for vertical main main menu
	 */
	function dessau_select_vertical_main_menu_styles() {
		$menu_holder_styles = array();
		
		$menu_top_margin    = dessau_select_options()->getOptionValue( 'vertical_menu_top_margin' );
		$menu_bottom_margin = dessau_select_options()->getOptionValue( 'vertical_menu_bottom_margin' );
		
		if ( ! empty( $menu_top_margin ) ) {
			$menu_holder_styles['margin-top'] = dessau_select_filter_px( $menu_top_margin ) . 'px';
		}
		if ( ! empty( $menu_bottom_margin ) ) {
			$menu_holder_styles['margin-bottom'] = dessau_select_filter_px( $menu_bottom_margin ) . 'px';
		}
		...
		echo dessau_select_dynamic_css( $menu_holder_selector, $menu_holder_styles );
		
		// vertical menu 1st level style
		
		$first_level_styles = dessau_select_get_typography_styles( 'vertical_menu_1st' );
		
		$first_level_selector = array(
			'.qodef-header-vertical .qodef-vertical-menu > ul > li > a'
		);
		
		echo dessau_select_dynamic_css( $first_level_selector, $first_level_styles );
		
		$first_level_hover_styles = array();
		
		$first_level_hover_color = dessau_select_options()->getOptionValue( 'vertical_menu_1st_hover_color' );
		...
		echo dessau_select_dynamic_css( $first_level_hover_selector, $first_level_hover_styles );
		
		// vertical menu 2nd level style
		
		$second_level_styles = dessau_select_get_typography_styles( 'vertical_menu_2nd' );
		
		$second_level_selector = array(
			'.qodef-header-vertical .qodef-vertical-menu .second .inner > ul > li > a'
		);
		
		echo dessau_select_dynamic_css( $second_level_selector, $second_level_styles );
		
		$second_level_hover_styles = array();
		
		$second_level_hover_color = dessau_select_options()->getOptionValue( 'vertical_menu_2nd_hover_color' );
		...
		echo dessau_select_dynamic_css( $second_level_hover_selector, $second_level_hover_styles );
		
		// vertical menu 3rd level style
		
		$third_level_styles = dessau_select_get_typography_styles( 'vertical_menu_3rd' );
		
		$third_level_selector = array(
			'.qodef-header-vertical .qodef-vertical-menu .second .inner ul li ul li a'
		);
		
		echo dessau_select_dynamic_css( $third_level_selector, $third_level_styles );
		
		
		$third_level_hover_styles = array();
		
		$third_level_hover_color = dessau_select_options()->getOptionValue( 'vertical_menu_3rd_hover_color' );
		...
		echo dessau_select_dynamic_css( $third_level_hover_selector, $third_level_hover_styles );
	}
	
	add_action( 'dessau_select_style_dynamic', 'dessau_select_vertical_main_menu_styles' );
}