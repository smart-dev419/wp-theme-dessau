...
if ( ! function_exists( 'dessau_select_register_header_vertical_type' ) ) {
	/**
	 * This function is used to register header type class for header factory file
	 */
	function dessau_select_register_header_vertical_type( $header_types ) {
		$header_type = array(
			'header-vertical' => 'DessauSelect\Modules\Header\Types\HeaderVertical'
		);
...
if ( ! function_exists( 'dessau_select_init_register_header_vertical_type' ) ) {
	/**
	 * This function is used to wait header-function.php file to init header object and then to init hook registration function above
	 */
	function dessau_select_init_register_header_vertical_type() {
		add_filter( 'dessau_select_register_header_type_class', 'dessau_select_register_header_vertical_type' );
	}
	
	add_action( 'dessau_select_before_header_function_init', 'dessau_select_init_register_header_vertical_type' );
}

if ( ! function_exists( 'dessau_select_include_header_vertical_menu' ) ) {
	/**
	 * Registers additional menu navigation for theme
	 */
	function dessau_select_include_header_vertical_menu( $menus ) {
		$menus['vertical-navigation'] = esc_html__( 'Vertical Navigation', 'dessau' );
		
		return $menus;
	}
	
	if ( dessau_select_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_filter( 'dessau_select_register_headers_menu', 'dessau_select_include_header_vertical_menu' );
	}
}

if ( ! function_exists( 'dessau_select_register_header_vertical_widget_areas' ) ) {
	/**
	 * Registers additional widget areas for this header type
	 */
	function dessau_select_register_header_vertical_widget_areas() {
		register_sidebar(
			array(
				'id'            => 'qodef-vertical-area',
				'name'          => esc_html__( 'Header Vertical Widget Area', 'dessau' ),
	...
	if ( dessau_select_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_action( 'widgets_init', 'dessau_select_register_header_vertical_widget_areas' );
	}
}

if ( ! function_exists( 'dessau_select_get_header_vertical_widget_areas' ) ) {
	/**
	 * Loads header widgets area HTML
	 */
	function dessau_select_get_header_vertical_widget_areas() {
...
if ( ! function_exists( 'dessau_select_get_header_vertical_main_menu' ) ) {
	/**
	 * Loads vertical menu HTML
	 */
	function dessau_select_get_header_vertical_main_menu() {
		$menu_opening = dessau_select_options()->getOptionValue('vertical_menu_dropdown_opening');
		
		$params = array(
			'opening_class' => 'qodef-vertical-dropdown-'. ( $menu_opening !== '' ? $menu_opening : 'below' )
		);

		dessau_select_get_module_template_part( 'templates/vertical-navigation', 'header/types/header-vertical', '', $params );
	}
}

if ( ! function_exists( 'dessau_select_vertical_header_holder_class' ) ) {
	/**
	 * Return holder class
	 */
	function dessau_select_vertical_header_holder_class() {
...
if ( ! function_exists( 'dessau_select_header_vertical_per_page_custom_styles' ) ) {
	/**
	 * Return header per page styles
	 */
	function dessau_select_header_vertical_per_page_custom_styles( $style, $class_prefix, $page_id ) {
		...
		if ( ! empty( $header_area_style ) ) {
			$current_style .= dessau_select_dynamic_css( $header_area_selector, $header_area_style );
		}
	...
	add_filter( 'dessau_select_add_header_page_custom_style', 'dessau_select_header_vertical_per_page_custom_styles', 10, 3 );
}