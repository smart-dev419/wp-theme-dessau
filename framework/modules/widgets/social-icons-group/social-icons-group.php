...
class DessauSelectClassIconsGroupWidget extends DessauSelectWidget {
	public function __construct() {
		parent::__construct(
			'qodef_social_icons_group_widget',
			esc_html__( 'Dessau Social Icons Group Widget', 'dessau' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'dessau' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			dessau_select_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'dessau' ),
					'options' => array(
						''             => esc_html__( 'Default', 'dessau' ),
						'square-icons' => esc_html__( 'Square', 'dessau' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'dessau' ),
					'description' => esc_html__( 'This applies to the square layout', 'dessau' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'dessau' ),
						'light-skin' => esc_html__( 'Light Skin', 'dessau' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'dessau' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'dessau' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo dessau_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php dessau_select_inline_style( $icon_styles ) ?>
