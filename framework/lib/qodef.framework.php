...
class DessauSelectFramework {
	...
	private function __construct() {
		$this->qodeOptions         = DessauSelectOptions::get_instance();
		$this->qodeMetaBoxes       = DessauSelectMetaBoxes::get_instance();
		$this->qodeTaxonomyOptions = DessauSelectTaxonomyOptions::get_instance();
		$this->qodeUserOptions     = DessauSelectUserOptions::get_instance();
		$this->qodeDashboardOptions = DessauSelectDashboardOptions::get_instance();
	}
	...
	public function setSkin( DessauSelectSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class DessauSelectSkinManager
 *
...
class DessauSelectSkinManager {
	...
	/**
	 * @see DessauSelectSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = SELECT_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'dessau' );
...
/**
 * Class DessauSelectSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class DessauSelectSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( dessau_select_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo dessau_select_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'dessau' ) ?>';
		</script>
	<?php }
...
class DessauSelectOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $dessau_select_options ) ) {
			return $dessau_select_options[ $key ];
...
/*
   Class: DessauSelectAdminPage
   A class that initializes Select Admin Page
*/
class DessauSelectAdminPage implements iDessauSelectLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new DessauSelectFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iDessauSelectRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: DessauSelectMetaBoxes
...
/*
   Class: DessauSelectMetaBox
   A class that initializes Select Meta Box
*/
class DessauSelectMetaBox implements iDessauSelectLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $dessau_select_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: DessauSelectTaxonomyOptions
   A class that initializes DessauSelect Taxonomy Options
*/
class DessauSelectTaxonomyOptions {
	private static $instance;
...
/*
   Class: DessauSelectTaxonomyOption
   A class that initializes DessauSelect Taxonomy Option
*/
class DessauSelectTaxonomyOption implements iDessauSelectLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new DessauSelectTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: DessauSelectUserOptions
   A class that initializes DessauSelect User Options
*/
class DessauSelectUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: DessauSelectUserOption
   A class that initializes DessauSelect User Option
*/
class DessauSelectUserOption implements iDessauSelectLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new DessauSelectUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: DessauSelectDashboardOptions
   A class that initializes DessauSelect Dashboard Options
*/
class DessauSelectDashboardOptions {
	private static $instance;
...
/*
   Class: DessauSelectDashboardOption
   A class that initializes DessauSelect Dashboard Option
*/
class DessauSelectDashboardOption implements iDessauSelectLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new DessauSelectDashboardFieldFactory();
	}
...
if ( ! function_exists( 'dessau_select_init_framework_variable' ) ) {
	function dessau_select_init_framework_variable() {
		global $dessau_select_Framework;
		
		$dessau_select_Framework = DessauSelectFramework::get_instance();
		$qodeSkinManager                 = new DessauSelectSkinManager();
		$dessau_select_Framework->setSkin( $qodeSkinManager->getSkin() );
	}
	
	add_action( 'dessau_select_before_options_map', 'dessau_select_init_framework_variable' );
}
?>