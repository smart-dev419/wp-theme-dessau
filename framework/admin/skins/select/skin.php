...
class SelectSkin extends DessauSelectSkinAbstract {
    /**
     * Skin constructor. Hooks to dessau_select_admin_scripts_init and dessau_select_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'select';

        //hook to admin register scripts
        add_action('dessau_select_admin_scripts_init', array($this, 'registerStyles'));
        add_action('dessau_select_admin_scripts_init', array($this, 'registerScripts'));

        //hook to admin enqueue scripts
        add_action('dessau_select_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('dessau_select_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('dessau_select_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('dessau_select_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			dessau_select_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            dessau_select_register_skin_style($styleHandle, $stylePath);
        }
    }
    /**
	...
	public function renderOptions() {
        ...
        $tab    = dessau_select_get_admin_tab();
        $active_page = $dessau_select_Framework->qodeOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see DessauSelectSkinAbstract::loadTemplatePart()
     */
     ...
     *
	 * @param DessauSelectAdminPage $page current page to load
     *
     * @see DessauSelectSkinAbstract::loadTemplatePart()
     */
